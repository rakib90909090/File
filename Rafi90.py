...
folder_path = '/sdcard/AUTO-KALYAN'
try:
...
ll=str([hari,tanggal])
kk=str([xy,co])
hx=(f"""  [bold green1]SEND BY[medium_purple1]   ⟩ [bold green1] KALYAN KING
  [bold green1]FACEBOOK[medium_purple1]    ⟩ [bold green1]BRYX ANTON GRAYSON
  [bold green1]TOOL/STATUS[medium_purple1] ⟩ [bold green1] FREE 
  [bold green1]TG : OX CYBER TEAM[medium_purple1]  ⟩ [bold green1]{ll}
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            dn()
            open("/sdcard/AUTO-KALYAN-SUCCESS-OK-ID.txt","a").write(uid+f"|{passw}|"+cookie+"\n")
            Ok+=1
    except Exception as e:
        pass
